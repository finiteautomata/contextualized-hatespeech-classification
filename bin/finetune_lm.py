...
allowed_contexts = {"none", "text", "title", "body"}
...
class DataTrainingArguments:
    ...
    context: str = field(
        metadata={"help": "What kind of context you should use."}
    )

    data_path: str = field(
        metadata={"help": "Path to JSON files."}
    )

    datasets_cache_dir: Optional[str] = field(
        default="/content/drive/MyDrive/.cache/huggingface",
        metadata={"help": "Path to JSON files."},
    )
    ...
    def __post_init__(self):
        if self.context not in allowed_contexts:
            raise ValueError(
                f"--context should be one of {allowed_contexts}",
            )
...
def main():
    ...
    logger.info(f"Loading files from {data_args.data_path}")

    files = sorted(glob(
        os.path.join(data_args.data_path, "*.json")
    ))

    train_files, test_files = files[:-5], files[-5:]
    ...
    def tokenize_function(examples):
        ...
        if context == "none":
            ...
            args = (examples["text"], examples["article_text"])
        elif context == "title":
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        ...
        logger.info(f"Context = {data_args.context}")
